...
from orbit.utils.dots.legacy import LEGACY_META_PKG, detect_legacy_repo
from orbit.utils.dots.packages import ArchInstaller
from orbit.utils.dots.source import DotsSource, SourceError
from orbit.utils.dots.state import DotsState
from orbit.utils.paths import config_dir

PKGS = ("orbit-shell", "orbit-cli", "quickshell")
INDENT = "    "
...
def print_legacy_install(meta_package: tuple[str, str] | None) -> None:
    ...
    update_msg = "Please update the CLI to the latest version and run 'orbit install' to update the dots."
    if sys.stdout.isatty():
...
def print_version() -> None:
    ...
    print()
    try:
        shell_ver = subprocess.check_output(["/usr/lib/orbit/version", "-s"], text=True).strip()
        _header("Shell:")
    ...
    local_shell_dir = config_dir / "quickshell/orbit"
