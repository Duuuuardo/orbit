...
from orbit.utils.paths import config_dir, data_dir

LEGACY_META_PKG = "orbit-meta"
...
def _find_legacy_repo(path: Path) -> Path | None:
    ...
    if remote.strip() != "https://github.com/orbit-dots/orbit.git":
        return
...
def detect_legacy_repo() -> Path | None:
    ...
    return _find_legacy_repo(data_dir / "orbit")
...
def legacy_symlinks(legacy_dir: Path | None) -> list[Path]:
    ...
    extras = [
        *(Path.home() / ".zen").glob("*/chrome/userChrome.css"),
        Path.home() / ".local/lib/orbit/orbitfox",
    ]
...
def legacy_to_delete(legacy_dir: Path | None) -> list[Path]:
    ...
    non_syms = [Path.home() / ".mozilla/native-messaging-hosts/orbitfox.json"]
