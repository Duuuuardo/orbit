...
from orbit.utils.dots.deployer import Deployer
from orbit.utils.dots.legacy import (
    LEGACY_META_PKG,
    detect_legacy_repo,
...
from orbit.utils.dots.manifest import ComponentError, Manifest, ManifestError
from orbit.utils.dots.misc import build_local_packages, run_hooks
from orbit.utils.dots.packages import DEFAULT_AUR_HELPER, PackageError, PackageInstaller
from orbit.utils.dots.source import DotsSource, SourceError
from orbit.utils.dots.state import DotsState
from orbit.utils.io import confirm, disable_input, fatal, info, log, pause, prompt_selection, warn
from orbit.utils.paths import (
    config_backup_dir,
    config_dir,
)
...
class Command:
    ...
    def print_greeting(self) -> None:
        ...
        info("Welcome to the Orbit dotfiles installer!")
    ...
    def migrate_legacy(self, installer: PackageInstaller, legacy_dir: Path | None) -> None:
        ...
        print()
        log("Found a legacy Orbit installation...")
    ...
    def print_done(self) -> None:
        print()
        info("All done! Orbit has been installed.")
        info("A few things to finish up:")
        info("  - A reboot is recommended for all changes to take effect")
        info("  - Edit `~/.config/orbit/hypr-vars.lua` to set default apps, keybinds and much more")
        info("  - Edit `~/.config/orbit/hypr-user.lua` to set your monitor layout and other Hyprland configs")
        info("  - Run `orbit update` later to pull in the latest changes")
        info("Enjoy! For support (or to just hang out), join our Discord server: https://orbitshell.com/discord")
