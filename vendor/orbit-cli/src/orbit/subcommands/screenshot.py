...
from orbit.utils import hypr
from orbit.utils.notify import notify
from orbit.utils.paths import screenshots_cache_dir, screenshots_dir
...
class Command:
    ...
    def region(self) -> None:
        if self.args.region == "slurp":
            subprocess.run(
                ["qs", "-c", "orbit", "ipc", "call", "picker", "openFreeze" if self.args.freeze else "open"]
            )
        else:
