...
MouseArea {
    ...
    function save(): void {
        const tmpfile = Qt.resolvedUrl(`/tmp/orbit-picker-${Quickshell.processId}-${Date.now()}.png`);
        CUtils.saveItem(screencopy, tmpfile, Qt.rect(Math.ceil(rsx), Math.ceil(rsy), Math.floor(sw), Math.floor(sh)), path => {
            if (root.loader.clipboardOnly) {
                ...
                Quickshell.execDetached(["notify-send", "-a", "orbit-cli", "-i", path, Tr.tr("Screenshot taken"), Tr.tr("Screenshot copied to clipboard")]);
            } else {
