...
class Tokens : public QQuickAttachedPropertyPropagator, public QQmlParserStatus {
    ...
    Q_PROPERTY(const orbit::config::AppearanceRounding* rounding READ rounding NOTIFY sourceChanged)
    Q_PROPERTY(const orbit::config::AppearanceSpacing* spacing READ spacing NOTIFY sourceChanged)
    Q_PROPERTY(const orbit::config::AppearancePadding* padding READ padding NOTIFY sourceChanged)
    Q_PROPERTY(const orbit::config::AppearanceTransparency* transparency READ transparency NOTIFY sourceChanged)
    Q_PROPERTY(const orbit::config::SizeTokens* sizes READ sizes NOTIFY sourceChanged)
    Q_PROPERTY(const orbit::config::FontTokens* font READ font NOTIFY sourceChanged)
    Q_PROPERTY(const orbit::config::AnimTokens* anim READ anim NOTIFY sourceChanged)
