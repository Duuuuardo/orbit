...
#define SETTINGS_NODE_NO_CTOR(Class, Base)                                                                             \
    Q_OBJECT                                                                                                           \
                                                                                                                       \
public:                                                                                                                \
    [[nodiscard]] const orbit::settings::Schema& schema() const override {                                         \
        static const auto schema =                                                                                     \
            orbit::settings::Schema::build(&staticMetaObject, Base::staticMetaObject.propertyCount());             \
        return schema;                                                                                                 \
    }                                                                                                                  \
                                                                                                                       \
private:                                                                                                               \
...
public:                                                                                                                \
    ...
    void set_##name(const Type& value) {                                                                               \
        ...
        if (forwardGlobalWrite(QStringLiteral(#name), QVariant::fromValue(value)))                                     \
            return;                       \
                                                                                                                       \
        const auto needsNotify = !orbit::settings::detail::compare(value, m_##name);                               \
        m_##name = value;                                                                                              \
    ...
    Q_SIGNAL void name##Changed();                                                                                     \
                                                                                                                       \
private:                                                                                                               \
    Type m_##name = fallbackValue(&Self::m_##name, orbit::settings::DefaultSpec::resolve<Type>(this, defaultVal)); \
    inline static const bool s_register_##name =                                                                       \
        (orbit::settings::Schema::annotate(&staticMetaObject, QStringLiteral(#name),                               \
             { .defaultValue = orbit::settings::DefaultSpec::create<Type>(defaultVal), __VA_ARGS__ }),             \
            true);
...
private:                                                                                                               \
    ...
    inline static const bool s_register_##name =                                                                       \
        (orbit::settings::Schema::annotate(                                                                        \
...
#define SETTINGS_LIST_TYPE(Element, Name)                                                                              \
    class Name : public orbit::settings::ListNode {                                                                \
        Q_OBJECT                                                                                                       \
        QML_ANONYMOUS                                                                                                  \
                                                                                                                       \
    public:                                                                                                            \
        explicit Name(Name* fallback = nullptr, QObject* parent = nullptr, bool globalOnly = false)                    \
            : orbit::settings::ListNode(fallback, parent, globalOnly) {}                                           \
                                                                                                                       \
        ...
        [[nodiscard]] Q_INVOKABLE Element* insert(const QVariantMap& props, qsizetype index = -1) {                    \
            return static_cast<Element*>(insertElement(props, index));                                                 \
                                                                                                                       ...
                                                                                                                       \
    protected:                                                                                                         \
        [[nodiscard]] orbit::settings::Node* createElement(orbit::settings::Node* fallback) override {         \
...
private:                                                                                                               \
    ...
    inline static const bool s_register_##name =                                                                       \
        (orbit::settings::Schema::annotate(&staticMetaObject, QStringLiteral(#name),                               \
             { .defaultValue = orbit::settings::DefaultSpec::create<QList<QVariantMap>>(defaultVal),               \
                 .globalOnly = global,                                                                                 \
                 __VA_ARGS__ }),                                                                                       \
            true);
