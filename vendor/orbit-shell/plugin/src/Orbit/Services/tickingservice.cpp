...
TickingService::TickingService(QObject* parent)
    ...
    auto* dash = orbit::config::ConfigSingleton::instance()->dashboard();
    applyInterval(dash->resourceUpdateInterval());
    QObject::connect(dash, &orbit::config::DashboardConfig::resourceUpdateIntervalChanged, this, [this, dash] {
        applyInterval(dash->resourceUpdateInterval());
    });
}
