...
Q_LOGGING_CATEGORY(lcLyrics, "orbit.lyrics", QtInfoMsg)
...
[[nodiscard]] const QHash<QByteArray, QByteArray>& lrclibHeaders() {
    static const QHash<QByteArray, QByteArray> k_h = {
        { "User-Agent"_ba, "orbit-shell (https://github.com/orbit-dots/shell)"_ba },
    };
    return k_h;
}
...
const QString& Lyrics::stateDir() {
    static const QString k_dir = [] {
        ...
        return state + u"/orbit/lyrics"_s;
    }();
    return k_dir;
}
...
const QString& Lyrics::cacheDir() {
    static const QString k_dir = [] {
        ...
        return cache + u"/orbit/lyrics"_s;
    }();
    return k_dir;
}
