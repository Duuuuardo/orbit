...
Q_LOGGING_CATEGORY(lcAcWorker, "orbit.services.ac.worker", QtInfoMsg)
...
PipeWireWorker::PipeWireWorker(std::stop_token token, AudioCollector* collector)
    ...
    m_stream = pw_stream_new_simple(pw_main_loop_get_loop(m_loop), "orbit-shell", props, &events, this);
    if (!m_stream) {
