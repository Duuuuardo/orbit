...
Errors, which break the string for translators:
  ...
  - QML using Tr without importing Orbit.I18n
...
SKIPPED = ("plugin/src/Orbit/I18n", "plugin/include/util/i18n.hpp")
...
IMPORT_RE = re.compile(r"^\s*import\s+Orbit\.I18n\b", re.M)
...
class FileChecker:
    ...
    def check_qml_imports(self) -> None:
        ...
        if use and not IMPORT_RE.search(self.src):
            self.report(
                (use.start(), use.end()), ERROR, "missing-import", "uses Tr but is missing `import Orbit.I18n`"
            )
